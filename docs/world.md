...
Delightify-level 把一个整合包实例的**运行时最终态**做成 agent 能查询的世界，以及一套确定性工具。思考、规划、改手写脚本发生在作者自己的 harness 里。
...
**工具**：检索、usages / neighbors / path、blast、unify 查询、engine dry-run、KubeJS preview。export / revert 只写 Delightify-level 受管文件，须由 harness 确认。
